...
tabell_20_kommune <- read_delim("datauttrekk_raw/tabell_20_kommune.csv", ";", escape_double = FALSE, 
...
tabell_20_bydel <- read_delim("datauttrekk_raw/tabell_20_bydel.csv", ";", escape_double = FALSE, 
...
temp = select(tabell_20_kommune, Region = KOKkommuneregion0000, Tid, Value = value) %>%
  bind_rows(., select(tabell_20_bydel, Region = KOKbydelsregion0000, Tid, Value = value))
...
temp = mutate(temp, 
              Variabel = "v20_barn_022_252_ilaaar",
              Tallformat = "Antall"
)
...
write.csv2(temp, "data_bearbeida/v20_barn_022_252_ilaaar.csv", row.names = FALSE)
...
rm(historiske_bydelsnr, historiske_fylkesnr, historiske_kommunekoder,
   ...
   metadata_geografi_alle, metadata_iks, tabell_20_bydel, tabell_20_kommune,
