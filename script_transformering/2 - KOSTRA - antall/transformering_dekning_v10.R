...
tabell_10_kommune <- read_delim("datauttrekk_raw/tabell_10_kommune.csv", ";", escape_double = FALSE, 
...
tabell_10_bydel <- read_delim("datauttrekk_raw/tabell_10_bydel.csv", ";", escape_double = FALSE, 
...
temp = select(tabell_10_kommune, Region = KOKkommuneregion0000, Tid, Value = value) %>%
  bind_rows(., select(tabell_10_bydel, Region = KOKbydelsregion0000, Tid, Value = value))
...
temp = mutate(temp, 
              Variabel = "v10_barn_undersokelse_tiltak_ilaaret_0_17",
              Tallformat = "Antall"
)
...
write.csv2(temp, "data_bearbeida/v10_barn_undersokelse_tiltak_ilaaret_0_17.csv", row.names = FALSE)
...
rm(historiske_bydelsnr, historiske_fylkesnr, historiske_kommunekoder,
   ...
   metadata_geografi_alle, metadata_iks, tabell_10_bydel, tabell_10_kommune,
