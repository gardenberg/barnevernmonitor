...
#TABELL 16 - KOMPRIMERT

#1 laste inn data
tabell_16_kommune <- read_delim("datauttrekk_raw/tabell_16_kommune.csv", ";", escape_double = FALSE, 
                               col_types = cols(KOKkommuneregion0000 = col_character(),
                                                KOKfunksjon0000 = col_character(),
                                                KOKart0000 = col_character(),
...
tabell_16_bydel <- read_delim("datauttrekk_raw/tabell_16_bydel.csv", ";", escape_double = FALSE, 
...
tabell_16_bydel = group_by(tabell_16_bydel, KOKbydelsregion0000, Tid) %>%
  summarise(value = sum(value)) #hvis noen av cellene er missing, bli hele verdien missing

#3 SLÅ SAMMEN SEPARATE SPØRRINGER ETTER SAMME VARIABEL
temp = select(tabell_16_kommune, Region = KOKkommuneregion0000, Tid, Value = value) %>%
  bind_rows(., select(tabell_16_bydel, Region = KOKbydelsregion0000, Tid, Value = value))
...
#dette er ikke tilfellet for KOSTRA-datasettene, som i stedet returnerer NA-verdier for ikke-aktuelle 
...
#HVIS KOSTRA-DATA
#KOSTRA-DATA HAR KOSTRA-GRUPPER EKG og EAKUO - landet uten Oslo, som ikke trengs. Disse fjernes
...
#HVIS KOSTRA
#KOSTRA HAR EAK og EAKUO, som er hele landet og hele landet uten oslo | #EKA01, EKA02 osv som er fylkene - skal omkodes | EKG som er KOSTRA-grupper
...
#i temp_omkodet ligger nå summerte tall for alle kommuner som har endret kommunenummer eller sammenslått
#dvs. at jeg tar ut NA-verdier, ikke-sammenslåtte verdier
...
#visuell inspeksjon viser at enhetne som ikke finnes i datasettet er en del bydeler, uoppgitt, IKS og Longyearbyen.
...
temp = mutate(temp, 
              Variabel = "v16_netto_driftsutgifter_1000kr_totalt_barnevern",
              Tallformat = "Antall"
)
...
write.csv2(temp, "data_bearbeida/v16_netto_driftsutgifter_1000kr_totalt_barnevern.csv", row.names = FALSE)
...
rm(historiske_bydelsnr, historiske_fylkesnr, historiske_kommunekoder,
   ...
   metadata_geografi_alle, metadata_iks, tabell_16_bydel, tabell_16_kommune,
