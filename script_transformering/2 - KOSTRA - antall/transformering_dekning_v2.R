...
tabell_2_kommune <- read_delim("datauttrekk_raw/tabell_2_kommune.csv", ";", escape_double = FALSE, 
...
tabell_2_bydel <- read_delim("datauttrekk_raw/tabell_2_bydel.csv", ";", escape_double = FALSE, 
...
temp = select(tabell_2_kommune, Region = KOKkommuneregion0000, Tid, Value = value) %>%
  bind_rows(., select(tabell_2_bydel, Region = KOKbydelsregion0000, Tid, Value = value))
...
temp = mutate(temp, 
              Variabel = "v2_barn_tiltak_ilaaret_ialt_0_17",
              Tallformat = "Antall"
)
...
write.csv2(temp, "data_bearbeida/v2_barn_tiltak_ilaaret_ialt_0_17.csv", row.names = FALSE)
...
rm(historiske_bydelsnr, historiske_fylkesnr, historiske_kommunekoder,
   ...
   metadata_geografi_alle, metadata_iks, tabell_2_bydel, tabell_2_kommune,
