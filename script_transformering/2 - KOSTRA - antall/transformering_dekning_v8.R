...
# TIL #1E Andel barn med hjelpetiltak i hjemmet av alle barn med tiltak 0-17 år 
...
tabell_8_kommune <- read_delim("datauttrekk_raw/tabell_8_kommune.csv", ";", escape_double = FALSE, 
...
tabell_8_bydel <- read_delim("datauttrekk_raw/tabell_8_bydel.csv", ";", escape_double = FALSE, 
...
temp = select(tabell_8_kommune, Region = KOKkommuneregion0000, Tid, Value = value) %>%
  bind_rows(., select(tabell_8_bydel, Region = KOKbydelsregion0000, Tid, Value = value))
...
temp = mutate(temp, 
              Variabel = "v8_barn_tiltak_ilaaret_251_0_17",
              Tallformat = "Antall"
)
...
write.csv2(temp, "data_bearbeida/v8_barn_tiltak_ilaaret_251_0_17.csv", row.names = FALSE)
...
rm(historiske_bydelsnr, historiske_fylkesnr, historiske_kommunekoder,
   ...
   metadata_geografi_alle, metadata_iks, tabell_8_bydel, tabell_8_kommune,
