...
# TIL # 2C Utgifter pr. barn som bor utenfor hjemmet

#ANTALL - KOSTRA

#STEG 1 1 laste inn data
tabell_21_kommune <- read_delim("datauttrekk_raw/tabell_21_kommune.csv", ";", escape_double = FALSE, 
...
tabell_21_bydel <- read_delim("datauttrekk_raw/tabell_21_bydel.csv", ";", escape_double = FALSE, 
...
temp = select(tabell_21_kommune, Region = KOKkommuneregion0000, Tid, Value = value) %>%
  bind_rows(., select(tabell_21_bydel, Region = KOKbydelsregion0000, Tid, Value = value))
...
temp = mutate(temp, 
              Variabel = "v21_brutto_driftsutgifter_252_1000kr",
              Tallformat = "Antall"
)
...
write.csv2(temp, "data_bearbeida/v21_brutto_driftsutgifter_252_1000kr.csv", row.names = FALSE)
...
rm(historiske_bydelsnr, historiske_fylkesnr, historiske_kommunekoder,
   ...
   metadata_geografi_alle, metadata_iks, tabell_21_bydel, tabell_21_kommune,
