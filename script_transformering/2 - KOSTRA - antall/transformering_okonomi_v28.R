...
# TIL 2F . Utgifter pr. barn 0-22 år som får tiltak i hjemmet (251)
...
tabell_28_kommune <- read_delim("datauttrekk_raw/tabell_28_kommune.csv", ";", escape_double = FALSE, 
...
tabell_28_bydel <- read_delim("datauttrekk_raw/tabell_28_bydel.csv", ";", escape_double = FALSE, 
...
temp = select(tabell_28_kommune, Region = KOKkommuneregion0000, Tid, Value = value) %>%
  bind_rows(., select(tabell_28_bydel, Region = KOKbydelsregion0000, Tid, Value = value))
...
temp = mutate(temp, 
              Variabel = "v28_brutto_driftsutgifter_251_1000kr",
              Tallformat = "Antall"
)
...
write.csv2(temp, "data_bearbeida/v28_brutto_driftsutgifter_251_1000kr.csv", row.names = FALSE)
...
rm(historiske_bydelsnr, historiske_fylkesnr, historiske_kommunekoder,
   ...
   metadata_geografi_alle, metadata_iks, tabell_28_bydel, tabell_28_kommune,
