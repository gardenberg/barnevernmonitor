...
tabell_4_kommune <- read_delim("datauttrekk_raw/tabell_4_kommune.csv", ";", escape_double = FALSE, 
...
tabell_4_bydel <- read_delim("datauttrekk_raw/tabell_4_bydel.csv", ";", escape_double = FALSE, 
...
temp = select(tabell_4_kommune, Region = KOKkommuneregion0000, Tid, Value = value) %>%
  bind_rows(., select(tabell_4_bydel, Region = KOKbydelsregion0000, Tid, Value = value))
...
write.csv2(temp, "data_bearbeida/v4_barn_undersokelse.csv", row.names = FALSE)
...
rm(historiske_bydelsnr, historiske_fylkesnr, historiske_kommunekoder,
   ...
   metadata_geografi_alle, metadata_iks, tabell_4_bydel, tabell_4_kommune,
