...
tabell_53_kommune <- read_delim("datauttrekk_raw/tabell_53_kommune.csv", ";", escape_double = FALSE, 
...
tabell_53_bydel <- read_delim("datauttrekk_raw/tabell_53_bydel.csv", ";", escape_double = FALSE, 
...
temp = select(tabell_53_kommune, Region = KOKkommuneregion0000, Tid, Value = value) %>%
  bind_rows(., select(tabell_53_bydel, Region = KOKbydelsregion0000, Tid, Value = value))
...
temp = mutate(temp, 
              Variabel = "v53_netto_driftsutgifter_forebygging_mm",
              Tallformat = "Antall"
)
...
write.csv2(temp, "data_bearbeida/v53_netto_driftsutgifter_forebygging_mm.csv", row.names = FALSE)
...
rm(historiske_bydelsnr, historiske_fylkesnr, historiske_kommunekoder,
   ...
   metadata_geografi_alle, metadata_iks, tabell_53_bydel, tabell_53_kommune,
