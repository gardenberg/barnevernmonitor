...
# TIL 2D - Andel av barnevernsutgiftene som går til saksbehandling
...
tabell_23_kommune <- read_delim("datauttrekk_raw/tabell_23_kommune.csv", ";", escape_double = FALSE, 
...
tabell_23_bydel <- read_delim("datauttrekk_raw/tabell_23_bydel.csv", ";", escape_double = FALSE, 
...
temp = select(tabell_23_kommune, Region = KOKkommuneregion0000, Tid, Value = value) %>%
  bind_rows(., select(tabell_23_bydel, Region = KOKbydelsregion0000, Tid, Value = value))
...
temp = mutate(temp, 
              Variabel = "v23_netto_driftsutgifter_244_1000kr",
              Tallformat = "Antall"
)
...
write.csv2(temp, "data_bearbeida/v23_netto_driftsutgifter_244_1000kr.csv", row.names = FALSE)
...
rm(historiske_bydelsnr, historiske_fylkesnr, historiske_kommunekoder,
   ...
   metadata_geografi_alle, metadata_iks, tabell_23_bydel, tabell_23_kommune,
