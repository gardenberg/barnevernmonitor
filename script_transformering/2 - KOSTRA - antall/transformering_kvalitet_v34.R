...
#34	Antall stillinger i barneverntjenesten med fagutdanning
...
tabell_34_kommune <- read_delim("datauttrekk_raw/tabell_34_kommune.csv", ";", escape_double = FALSE, 
...
tabell_34_bydel <- read_delim("datauttrekk_raw/tabell_34_bydel.csv", ";", escape_double = FALSE, 
...
temp = select(tabell_34_kommune, Region = KOKkommuneregion0000, Tid, Value = value) %>%
  bind_rows(., select(tabell_34_bydel, Region = KOKbydelsregion0000, Tid, Value = value))
...
temp = mutate(temp, 
              Variabel = "v34_antall_stillinger_fagutdanning",
              Tallformat = "Antall"
)
...
write.csv2(temp, "data_bearbeida/v34_antall_stillinger_fagutdanning.csv", row.names = FALSE)
...
rm(historiske_bydelsnr, historiske_fylkesnr, historiske_kommunekoder,
   ...
   metadata_geografi_alle, metadata_iks, tabell_34_bydel, tabell_34_kommune,
