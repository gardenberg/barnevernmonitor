...
tabell_58_kommune <- read_delim("datauttrekk_raw/tabell_58_kommune.csv", ";", escape_double = FALSE, 
...
tabell_58_bydel <- read_delim("datauttrekk_raw/tabell_58_bydel.csv", ";", escape_double = FALSE, 
...
temp = select(tabell_58_kommune, Region = KOKkommuneregion0000, Tid, Value = value) %>%
  bind_rows(., select(tabell_58_bydel, Region = KOKbydelsregion0000, Tid, Value = value))
...
temp = mutate(temp, 
              Variabel = "v58_antall_barn_bekymringsmelding",
              Tallformat = "Antall"
)
...
write.csv2(temp, "data_bearbeida/v58_antall_barn_bekymringsmelding.csv", row.names = FALSE)
...
rm(historiske_bydelsnr, historiske_fylkesnr, historiske_kommunekoder,
   ...
   metadata_geografi_alle, metadata_iks, tabell_58_bydel, tabell_58_kommune,
