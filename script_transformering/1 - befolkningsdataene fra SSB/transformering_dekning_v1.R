...
#VARIABLER
# 1	Antall innbyggere fra og med 0 til og med 17 år	https://www.ssb.no/statbank/table/10826/ (B),  https://www.ssb.no/statbank/table/07459/ (K)

#Til 2A - Utgifter til barnevern pr. barn i befolkningen 0-17 år 

#1 laste inn data igjen
#TODO: legge inn eksplisitte definisjoner av typer
tabell_1_kommune <- read_delim("datauttrekk_raw/tabell_1_kommune.csv", ";", escape_double = FALSE, 
                               col_types = cols(Region = col_character(),
                                                Tid = col_character()), 
                               locale = locale(decimal_mark = ",", grouping_mark = " "), 
                               trim_ws = TRUE)

tabell_1_bydel <- read_delim("datauttrekk_raw/tabell_1_bydel.csv", ";", escape_double = FALSE, 
                               col_types = cols(Region = col_character(),
                                                Tid = col_character()), 
                               locale = locale(decimal_mark = ",", grouping_mark = " "), 
                               trim_ws = TRUE)
...
temp = bind_rows(tabell_1_kommune, tabell_1_bydel)
...
##ssb_07459 og ssb_10826 har avvikende bydelskoding - 030101a fra og med 2004. 
#Fra 1. januar 2004 fikk kommunene Oslo og Trondheim ny bydelsinndeling.
#dvs. at de tomme observasjonene med bydeler som har riktige koder må fjernes, og erstattes med de gjeldende bydelene, som må omkodes til riktige koder...

temp = mutate(temp, Region = gsub("a", "", Region))

#her bør det ikke være noen duplikater? 
#hver rad i datasettet bør være en observasjon pr. region pr. tid. 
#sjekker ved å sammenholde unike kombinasjoner av region og tid, med radantallet 
test = get_dupes(temp, Region, Tid)

test = distinct(temp, Region) %>%
  anti_join(., metadata_geografi_alle, by = c("Region" = "code"))

#fjerner duplikater av store kommuner fra bydelsfila.
temp = filter(temp, !(Region %in% c("030100", "110300", "120100", "160100", "460100", "500100")))

#6 OMKODING DEL 2 - KOMMUNEMAPPING (FOR SUMMERBARE VARIABLER)
#datasettet har nå historiske kommunekoder, dvs. gjeldende koder på tidspunktet for rapportering.
#vi ønsker et datasett med historiske serier for dagens kommuner, uavhengig av bytte av nummer, sammenslåing - eller splitting.
#for å få til dette tar jeg et datasett med alle dagens kommuner, og hva disse har vært kodet som historisk

#filtrerer vekk alle kommuner som ikke har endret nummer (som 0301 Oslo) - men må ha med kommuner som ikke har bytta
#nummer, men som har sammenslått seg (5001 Trondheim og 1103 Stavanger)
#får da også med Trønder-kommuner med flere oppføringer, men også en oppføring av samme kommunenummer.

#splittede kommuner: det er kommuner som har flere kommuner i kommunenummer_2020-lista enn i historiske-lista.
#det finner jeg vede å gruppere datasettet på historisk nummer, og telle opp antallet rader i hver gruppe
#da vil 1613/5012 få tre, fordi denne kommunen ble delt på 3, og 1850 får to, alle andre får 1.

#så kobler jeg på dataene fra datasettet til dette settet, slik at jeg har et datasett med verdier for kommuner som har byttet nummer
#for kommunene som kun har byttet nummer, er dette nok.

#før summering til nye kommuner, må jeg fordele tallet i splittede kommuner
#deler det ganske enkelt likt mellom de nye kommunene.

#så grupperer jeg på den nye koden og tid, for å få summert også koder for kommuner som har slått seg sammen.
#summerer variabelen.
...
#henter en tabell med fylker som har skiftet nummer (og navn)
#legger på verdiene i datasettet for disse
#så grupperer jeg på den nye koden og tid, for å få summert 
...
temp_iks = left_join(metadata_iks, temp, by = c("Kommunenummer" = "Region")) %>%
  ...
  summarise(Value = sum(Value, na.rm = FALSE)) %>% #hvis NA på en av verdiene, så blir det NA på hele - men ingen NA her.
  rename(Region = 1)
...
#lager liste med manglende kombinasjoner i data - gjelder KOSTRA-data med bydeler utenfor Oslo.
manglende_observasjoner = expand(temp, Region, Tid) %>%
  filter(is.na(Tid) == FALSE) %>%
  anti_join(temp)

#i tabell 1 er dette i hovedsak enheter som ikke finnes, som er fjerna fordi  de ikke finnes lenger opp
#men det er også
...
write.csv2(temp, "data_bearbeida/v1_antallbarn_0_17.csv", row.names = FALSE)
...
rm(historiske_bydelsnr, historiske_fylkesnr, historiske_kommunekoder,
   ...
   metadata_geografi_alle, metadata_iks, tabell_1_bydel, tabell_1_kommune,
