...
#321	Antall tilsynskrav i fosterhjem	KHR-kuben
#3B - Tilsynskrav i fosterhjem som ikke er oppfylt (antall og andel)

#ANTALL - KHR
#OBS! PRIKKING!

#opprettet 2. mars 2021
#versjon 001
...
#Det er også behov for ulik håndtering av KOSTRA-data og befolkningsdata, tall som vi lager sjøl, og filleveranser fra SSB og fylkesnemda og KHR
...
#fylker: januar 1972 ->  | #kode for hele landet: 0 |#regionale spesialkoder:  kun gjeldende for Longyearbyen pr februar 2021 og Svalbard 21
...
#DEN OVER ERSTATTES MED DENNE - OBS! NEI, TO FORSKJELLIGE FORMÅL MED DISSE LISTENE - EN ER FOR MAPPING; EN ANNA ER FOR GYLDIGHETSKONTROLL
historiske_kommunekoder_gyldig <- read_delim("metadata_geografi/kommunenummer_fra2013_til2020_lang_gyldighet.csv", ";", escape_double = FALSE, 
                                             ...
                                             locale = locale(encoding = "ISO-8859-1"), trim_ws = TRUE)
#OBS! Ikke filtrering på år her, KHR har data 2013-2020.
...
#TRENGER også en gyldighetsliste for denne
historiske_fylkesnr_gyldighet <- read_delim("metadata_geografi/fylkenummer_fra2013_til2019_lang_gyldighet.csv", ";", escape_double = FALSE,
                                  col_types = cols(fylkenr_2020 = col_character(),
                                                   historisk_nummer = col_character(),
                                                   gyldig_år = col_character()
                                                   ),
                                  locale = locale(encoding = "ISO-8859-1"), trim_ws = TRUE)

#STEG 1 - LASTE INN DATA

tabell_321 <- read_delim("data_manuelle_leveranser/KHR - tabell 32-321-33 uttrekk fra KHR til barnevernsmonitor 1-mars-2021.csv", ";", escape_double = FALSE, 
                        col_types = cols(Rapporteringsenhet = col_character(),
                                         `År` = col_character(), 
                                         `Halvår` = col_character(),
                                         `Fosthj Barn Plassert Mer Enn 1 År Tilsynskrav Ikke Oppfylt` = col_skip(),
                                         `Fosthj Barn Tilsyn Plassert Mer Enn 1 År` = col_number(),
                                         `Fosthj Andel barn plassert mer enn 1 år tilsynskrav ikke oppfylt` = col_skip()),
...
# tabell_25_bydel er ikke aggregert over funksjoner

#OBS: vurder å ha en sjekk av det her.

#EKSEMPEL
#tabell_25_bydel = group_by(tabell_25_bydel, KOKbydelsregion0000, Tid) %>%
#  summarise(value = sum(value, na.rm = TRUE))

#3 SLÅ SAMMEN SEPARATE SPØRRINGER ETTER SAMME VARIABEL
#KHR-dataene trenger ikke dette. De fleste andre gjør det, enten geografisk eller tidsmessig.
#temp = select(tabell_11_kommune, Region = KOKkommuneregion0000, Tid, Value = value) %>%
#  bind_rows(., select(tabell_11_bydel, Region = KOKbydelsregion0000, Tid, Value = value))

#STEG X - KODE OM VARIABELNAVN FOR 
#Variabelnavnene kodes om i sammenslåinga for de fleste - men ikke KHR
temp = rename(tabell_321,
                   Region = 1,
                   Tid = 2,
                   Halvår = 3,
                   Value = 4
...
#KHR: Her er det manglende data for år og variabler hvor en ikke har data, og for observasjoner som ikke har rapportert. 
#Første versjon: fjerner alle observasjoner som er manglende...? Nei, det klusser til IKS-beregningene, som må være missing hvis en av deltakerne er missing.
#Første versjon: fjerner tomme år og år som har 0 observasjoner, altså 2004-2012 og 2021 p.t.

test = group_by(temp, Tid, Halvår) %>%
  summarise(
    antall_observasjoner = n(),
    antall_na = sum(is.na(Value) == TRUE),
    andel_na = antall_na / antall_observasjoner,
    antall_saker = sum(Value, na.rm = TRUE)
  )


temp = group_by(temp, Tid, Halvår) %>%
  mutate(
    antall_observasjoner = n(),
    antall_na = sum(is.na(Value) == TRUE),
    andel_na = antall_na / antall_observasjoner,
  ...
  ungroup() %>%
  filter(andel_na != 1, antall_saker > 0) %>%
  select(-antall_observasjoner, -antall_na, -andel_na, -antall_saker)

#her støtte jeg på at enkelte variabler har lengre tidsserier. kutter dem til 2013-> 
temp = filter(temp, parse_number(Tid) > 2012)
...
#KHR-dataene har 3-sifrede region-koder pga manglende 0 først. Har ingen fylker eller landet, så det kan paddes til minimum fire bred.
#OBS: I enkelte KHR-uttrekk med kommunenummer-dimensjonen, ikke rapporteringsenhet-dimensjonen, fikk jeg manglende trailing 0 (161 i stedet for 1610 feks)
temp = mutate(temp,
              Region = str_pad(Region, 4, side = "left", pad = "0")
              )

#KHR har også bydeler i Oslo som 301BB, med manglende 0 først. Dette er de eneste Region-kodene som har 5 tegn i seg

test = filter(temp, nchar(Region) == 5) %>%
  distinct(Region)

temp = mutate(temp,
              Region = ifelse(nchar(Region) == 5, str_pad(Region, 6, side = "left", pad = "0"), Region)
              )
...
test = janitor::get_dupes(temp, Region, Tid, Halvår)

#KHR - BYDELSMAPPING.
test = distinct(temp, Region) %>%
  anti_join(., metadata_geografi_alle, by = c("Region" = "code"))

#KHR har koden 12010001 for EMA-tjenesten Bergen i 2017-2019, og 500105 i 2017-2019 for EMA-tjenesten Trondheim. 
#Disse er seinere lagt til uoppgitt bydel 120199 og 500199 - men disse kodene finnes allerede i datagrunnlaget.
#KHR har både koden 2111 og 2122.  Longyearbyen skal være 2111 i monitor - hva er 2122? Det kan  aggregeres til 2111 - men den finnes også i datagrunnlaget.

temp_bydel = filter(temp, 
                    Region %in% c("12010001", "500105", "2122")
                    ) %>%
  mutate(Region_feil = Region,
         Region = ifelse(Region_feil == "12010001", "120199", NA),
         Region = ifelse(Region_feil == "500105", "500199", Region),
         Region = ifelse(Region_feil == "2122", "2111", Region)
  ) %>%
  bind_rows(., filter(temp, Region %in% c("120199", "500199", "2111"))) %>%
  filter(is.na(Value) == FALSE) %>% #fjerner NA-verdier, for de skal være tomme.
  group_by(Region, Tid, Halvår) %>%
  summarise(
    Value = sum(Value) 
  )

test = bind_rows(temp, temp_bydel) %>%
  janitor::get_dupes(., Region, Tid, Halvår)
...
temp = anti_join(temp, temp_bydel, by = c("Region", "Tid", "Halvår")) %>%
  bind_rows(., temp_bydel)

#fjerner også Region_feil fra hoveddatasettet, for nå er de slått sammen med riktige bydeler
temp = filter(temp, !(Region %in% c("12010001", "500105", "2122")))

test = janitor::get_dupes(temp, Region, Tid, Halvår)

#KHR - BYKOMMUNEAGGREGERING.
#det finnes bydeler, som må aggregeres til sine hovedkommuner. 
#hvilke bydeler er det snakk om, i hvilke kommuner?

test = filter(temp, nchar(Region) > 4) %>%
  distinct(Region) %>%
  mutate(kommune = str_sub(Region, 1, 4))

#her er i hvert fall 0301 Oslo, 1103 Stavanger 1201 og 4601 Bergen, 1601 og 5001 Trondheim. Finnes disse kommunene i hoveddatasettet?

test = filter(temp, Region %in% test$kommune)

#noen på Stavanger 2017-2020 og Trondheim 2018

temp_bydel = filter(temp, nchar(Region) > 4) %>%
  mutate(
    Bydel = Region,
    Region = str_sub(Region, 1, 4)) %>%
  bind_rows(., filter(temp, Region %in% c("0301", "1103", "1201", "4601", "1601", "5001"))) %>%
  group_by(Region, Tid, Halvår) %>%
  summarise(
    Value = sum(Value, na.rm = TRUE) #her ignorerer jeg NA-verdier, fordi det kan antas å være 0, ikke manglende.
  )
...
#6 OMKODING DEL 2 - KOMMUNEMAPPING 
...
temp_omkodet = rename(historiske_kommunekoder_gyldig, Tid = gyldig_år) %>%
  ...
  group_by(kommunenummer_2020, Tid, Halvår) %>%
  mutate( 
...
test = bind_rows(temp, temp_omkodet) %>%
  janitor::get_dupes(., Region, Tid, Halvår, Value)

#fjerner duplikatene fra temp-fila
#dvs. at jeg tar ut NA-verdier, og for antall også sammenslåtte verdier?
#men tenk gjerne en gang til - er dette tilrådelig?
temp = anti_join(temp, temp_omkodet, by = c("Region", "Tid", "Halvår")) %>%
  bind_rows(., temp_omkodet)

test = janitor::get_dupes(temp, Region, Tid, Halvår)

#7 OMKODING - FYLKEMAPPING - IKKE MAPPING FOR KHR, AGGREGERING.
...
#visuell inspeksjon viser at enhetene som ikke finnes i KHR-datasettet er en del bydeler, uoppgitt, IKS og fylker.
...
temp_iks = left_join(metadata_iks, temp, by = c("Kommunenummer" = "Region")) %>%
  group_by(Samarbeidskommunenr, Tid, Halvår) %>%
...
#aggregerer fylker
#går ut ifra at kommunesettet i grunnlagsdataene er komplett, og at jeg dermed kan finne alle sammen derifra, ta fnr og summere
#men siden temp-datasettet nå har et variert utvalg av kommuner, 
#må det filtreres mot ei malliste for hva som er gyldig. 

#det gjør også at alle eksisterende manglende data må håndteres på en eller annen måte. Velger i første rekke å ignorere manglende data
#1613/5012 og 1850 - Snillfjord og Tysfjord, splittekommunene, må summeres inn på riktige fylker, 
#sammen med historikken til kommmunene de ble slått sammen med
#for å gjøre en lang historie kort, er det greiest å gå tilbake til tabell_30 og urørte data, og finne fylke basert på rapporteringsenhet.

temp_fylke = mutate(temp_umappet,
                    fnr = str_sub(Region, 1, 2)
                    ) %>%
  group_by(fnr, Tid, Halvår) %>%
  summarise(
...
checksum_start = sum(temp_fylke$Value)

test = janitor::get_dupes(temp_fylke, Region, Tid, Halvår)

#dette fylkesdatasettet har nå ikke lenger gyldige koder, som må mappes om. 

temp_fylke_mapping = left_join(historiske_fylkesnr_gyldighet, temp_fylke, by = c("historisk_nummer" = "Region", "gyldig_år" = "Tid")) %>%
  group_by(fylkenr_2020, gyldig_år, Halvår) %>%
  summarise(
    Value = sum(Value)
  ) %>%
  rename(
...
test = janitor::get_dupes(temp_fylke_mapping, Region, Tid, Halvår)

#tester hvordan dette blir - det bør være duplikater på alt for fylkene som ikke har bytta nummer i 2020 (03, 11, 15, 18 og 50)
test = bind_rows(temp_fylke, temp_fylke_mapping) %>%
  janitor::get_dupes(., Region, Tid, Halvår, Value)

#og så bør det være faktiske verdier for de øvrige
test = bind_rows(temp_fylke, temp_fylke_mapping) %>%
  janitor::get_dupes(., Region, Tid, Halvår)

#fjerner duplikatene fra temp_fylke-fila
#dvs. at jeg tar ut NA-verdier, og for antall også sammenslåtte verdier?
#men tenk gjerne en gang til - er dette tilrådelig?
temp_fylke = anti_join(temp_fylke, temp_fylke_mapping, by = c("Region", "Tid", "Halvår")) %>%
  bind_rows(., temp_fylke_mapping)

test = get_dupes(temp_fylke, Region, Tid, Halvår)

#nå bør checksummen hvis jeg tar de gyldige fylkene fra denne tabellen og summerer, være lik checksummen over.

test = filter(metadata_episerver, nchar(code) == 2) %>%
  select(Region = code) %>%
  left_join(., temp_fylke)

checksum_end = sum(test$Value, na.rm = TRUE)
checksum_start == checksum_end

#binder det inn i hoveddatasettet
temp = bind_rows(temp, temp_fylke)

#AGGREGERING AV HELE LANDET - 0

temp_landet = group_by(temp_umappet, Tid, Halvår) %>%
  summarise(
    Value = sum(Value, na.rm = TRUE)
  ) %>%
  mutate(
...
#binder det inn i hoveddatasettet
temp = bind_rows(temp, temp_landet)
...
#OBS! Ved innlesning på nytt må en huske å lese inn som tekst hvis en ønsker prikk.
...
manglende_observasjoner = expand(temp, Region, Tid, Halvår) %>%
  filter(is.na(Tid) == FALSE, is.na(Halvår) == FALSE) %>%
  anti_join(temp) %>%
  semi_join(., metadata_episerver, by = c("Region" = "code"))

test = bind_rows(temp, manglende_observasjoner) %>%
  get_dupes(Region, Tid, Halvår)

temp = bind_rows(temp, manglende_observasjoner) %>%
  filter(is.na(Tid) == FALSE, is.na(Halvår) == FALSE)

#PRIKKESTEGET MÅ VÆRE HER
#NB! BUG! Må den være her? Da prikker vi ikke IKS? Er det fordi det regnes med dataene, og de ikke kan være tekst?
#Skal vi prikke noe i grunnlagsdataene her?
#OBS! Kun tabell 32, 321 og 33 skal prikkes her.
#Vi prikker observasjoner som er 1, 2 eller 3. Settes først missing, så settes alt som er ikke-finite til ".." i neste steg

antall_obs_under_3 = sum(temp$Value %in% c(1, 2, 3), na.rm = TRUE)

temp = mutate(temp,
              Value = ifelse(Value %in% c(1, 2, 3), NA, Value)
)
...
test = get_dupes(temp, Region, Tid, Halvår)
test = summarise_all(temp, funs(sum(is.na(.))))
...
temp = mutate(temp, 
              Variabel = "v321_antall_tilsynskrav_fosterhjem",
              Tallformat = "Antall"
)
...
write.csv2(temp, "data_bearbeida/v321_antall_tilsynskrav_fosterhjem.csv", row.names = FALSE)

#rydder opp tabeller som ikke trenger å være i minnet lengre
rm(historiske_bydelsnr, historiske_fylkesnr, historiske_fylkesnr_gyldighet, historiske_kommunekoder,
   historiske_kommunekoder_gyldig, manglende_observasjoner, metadata_episerver,
   metadata_geografi_alle, metadata_iks, tabell_321, 
   temp, temp_bydel, temp_fylke, temp_fylke_mapping, temp_iks, temp_landet, temp_omkodet, temp_umappet, test,
   antall_obs_under_3, checksum_start, checksum_end
)
