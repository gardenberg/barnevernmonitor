...
#301	Antall Undersøkelser avsluttet siste halvår	KHR-kuben
#TIL 3A - Fristbrudd i undersøkelsessaker (antall og andel)
...
tabell_301 <- read_delim("data_manuelle_leveranser/KHR - tabell 30-301-31 uttrekk fra KHR til barnevernsmonitor 26-februar-2021.csv", ";", escape_double = FALSE, 
                        col_types = cols(Rapporteringsenhet = col_character(),
                                         `År` = col_character(), `Halvår` = col_character(),
                                         `Fosthj Fylkesmannens Vurd Fristovers Antall` = col_skip(),
                                         `Unders Avsl Siste Halvår` = col_number(),
                                         `Fosthj Fylkesmannens vurd fristovers andel` = col_skip()),
...
temp = mutate(temp, 
              Variabel = "v301_antall_undersøkelser_siste_halvår",
              Tallformat = "Antall"
)
...
write.csv2(temp, "data_bearbeida/v301_antall_undersøkelser_siste_halvår.csv", row.names = FALSE)
...
rm(historiske_bydelsnr, historiske_fylkesnr, historiske_fylkesnr_gyldighet, historiske_kommunekoder,
   ...
   metadata_geografi_alle, metadata_iks, tabell_301, 
