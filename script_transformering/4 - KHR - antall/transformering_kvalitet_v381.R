...
#381	Antall barn i hjelpetiltak	KHR-kuben
...
tabell_381 <- read_delim("data_manuelle_leveranser/KHR - tabell 38-381-39 uttrekk fra KHR til barnevernsmonitor 1-mars-2021.csv", ";", escape_double = FALSE, 
                        col_types = cols(Rapporteringsenhet = col_character(),
                                         `År` = col_character(), 
                                         `Halvår` = col_character(),
                                         `Tiltak Barn Hjtilt Med Tiltaksplan §4-5` = col_skip(),
                                         `Tiltak Barn Hjtilt § 4-4 Tot` = col_number(),
...
temp = mutate(temp, 
              Variabel = "v381_antall_barn_hjelpetiltak",
              Tallformat = "Antall"
)
...
write.csv2(temp, "data_bearbeida/v381_antall_barn_hjelpetiltak.csv", row.names = FALSE)
...
rm(historiske_bydelsnr, historiske_fylkesnr, historiske_fylkesnr_gyldighet, historiske_kommunekoder,
   ...
   metadata_geografi_alle, metadata_iks, tabell_381, 
