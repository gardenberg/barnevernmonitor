...
temp_tabell_45 <- read_delim("data_bearbeida/v45_antall_innbyggere_totalt.csv", ";", escape_double = FALSE, 
                                 col_types = cols(Region = col_character(),
                                                  Tid = col_character(),
                                                  Value = col_number(),
                                                  Variabel = col_skip(),
                             ...
                             na = "..",  
                                 trim_ws = TRUE) %>%
  rename(v45_antall_innbyggere_totalt = Value)
...
temp = full_join(temp_tabell_1, temp_tabell_45, by = c("Region", "Tid"))
...
temp = mutate(temp,
                    Value =
                      round((v1_antallbarn_0_17 / v45_antall_innbyggere_totalt)*100, 1)
                      ) %>%
  select(
    Region, 
    Tid, 
    Andel = Value,
    Antall = v1_antallbarn_0_17) %>%
...
temp = mutate(temp, Variabel = "4A_andel_barn")
...
write.csv2(temp, "data_til_pivotering/4A_andel_barn.csv", row.names = FALSE)
write.csv2(temp, "data_bearbeida/v46_andel_barn.csv", row.names = FALSE)

#rydder opp tabeller som ikke trenger å være i minnet lengre
rm(temp, temp_tabell_1, temp_tabell_45, test)
