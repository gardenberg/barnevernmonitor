...
# 3C - Antall stillinger i barneverntjenesten med fagutdanning pr. 1000 barn 0-17 år

#ANDEL - KOSTRA - IKKE ANTALL
...
tabell_35_kommune <- read_delim("datauttrekk_raw/tabell_35_kommune.csv", ";", escape_double = FALSE, 
...
tabell_35_bydel <- read_delim("datauttrekk_raw/tabell_35_bydel.csv", ";", escape_double = FALSE, 
...
temp = select(tabell_35_kommune, Region = KOKkommuneregion0000, Tid, Value = value) %>%
  bind_rows(., select(tabell_35_bydel, Region = KOKbydelsregion0000, Tid, Value = value))
...
temp_tabell_34 <- read_delim("data_bearbeida/v34_antall_stillinger_fagutdanning.csv", ";", escape_double = FALSE, 
                                 col_types = cols(Region = col_character(),
                                                  Tid = col_character(),
                                                  Value = col_number(),
                                                  Variabel = col_skip(),
                             ...
                             na = "..", 
                                 trim_ws = TRUE) %>%
  rename(v34_antall_stillinger_fagutdanning = Value)
...
temp = full_join(temp, temp_tabell_1, by = c("Region", "Tid")) %>%
  full_join(., temp_tabell_34, by = c("Region", "Tid"))
...
temp = mutate(temp,
                    Value =
                      ifelse(is.na(Value) == TRUE, 
                             round((v34_antall_stillinger_fagutdanning / v1_antallbarn_0_17)*1000, 1),
                             Value
                      )) %>%
  select(
    Region, 
...
temp = mutate(temp, Variabel = "3C_andel_stillinger_fagutdanning")
...
write.csv2(temp, "data_til_pivotering/3C_andel_stillinger_fagutdanning.csv", row.names = FALSE)
write.csv2(temp, "data_bearbeida/v35_andel_stillinger_fagutdanning.csv", row.names = FALSE)
...
rm(historiske_bydelsnr, historiske_fylkesnr, historiske_kommunekoder,
   ...
   metadata_geografi_alle, metadata_iks, tabell_35_bydel, tabell_35_kommune,
   temp, temp_bydel, temp_omkodet, temp_tabell_1, temp_tabell_34, test
)
