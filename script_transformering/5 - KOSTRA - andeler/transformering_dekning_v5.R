...
# Til 1B Barn med undersøkelse i forhold til antall innbyggere i aldersgruppen 0-17 år 
...
tabell_5_kommune <- read_delim("datauttrekk_raw/tabell_5_kommune.csv", ";", escape_double = FALSE, 
...
tabell_5_bydel <- read_delim("datauttrekk_raw/tabell_5_bydel.csv", ";", escape_double = FALSE, 
...
temp = select(tabell_5_kommune, Region = KOKkommuneregion0000, Tid, Value = value) %>%
  bind_rows(., select(tabell_5_bydel, Region = KOKbydelsregion0000, Tid, Value = value))
...
temp_tabell_4 <- read_delim("data_bearbeida/v4_barn_undersokelse.csv", ";", escape_double = FALSE, 
                                 col_types = cols(Region = col_character(),
                                                  Tid = col_character(),
                                                  Value = col_number(),
                                                  Variabel = col_skip(),
                            ...
                            na = "..",
                                 trim_ws = TRUE) %>%
  rename(v4_barn_undersokelse = Value)
...
temp = full_join(temp, temp_tabell_1, by = c("Region", "Tid")) %>%
  full_join(., temp_tabell_4, by = c("Region", "Tid"))
...
temp = mutate(temp,
                    Value =
                      ifelse(is.na(Value) == TRUE, 
                             round((v4_barn_undersokelse / v1_antallbarn_0_17)*100, 1),
                             Value
                      )) %>%
  select(
    Region, 
...
temp = mutate(temp, Variabel = "1B_andel_barn_undersokelse_0_17")
...
write.csv2(temp, "data_til_pivotering/1B_andel_barn_undersokelse_0_17.csv", row.names = FALSE)
write.csv2(temp, "data_bearbeida/v5_andel_barn_undersokelse_0_17.csv", row.names = FALSE)
...
rm(historiske_bydelsnr, historiske_fylkesnr, historiske_kommunekoder,
   ...
   metadata_geografi_alle, metadata_iks, tabell_5_bydel, tabell_5_kommune,
   temp, temp_bydel, temp_omkodet, temp_tabell_1, temp_tabell_4, test
)
