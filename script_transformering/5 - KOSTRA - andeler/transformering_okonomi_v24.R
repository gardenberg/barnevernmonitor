...
tabell_24_kommune <- read_delim("datauttrekk_raw/tabell_24_kommune.csv", ";", escape_double = FALSE, 
...
tabell_24_bydel <- read_delim("datauttrekk_raw/tabell_24_bydel.csv", ";", escape_double = FALSE, 
...
temp = select(tabell_24_kommune, Region = KOKkommuneregion0000, Tid, Value = value) %>%
  bind_rows(., select(tabell_24_bydel, Region = KOKbydelsregion0000, Tid, Value = value))
...
temp_tabell_23 <- read_delim("data_bearbeida/v23_netto_driftsutgifter_244_1000kr.csv", ";", escape_double = FALSE, 
                                 col_types = cols(Region = col_character(),
                                                  Tid = col_character(),
                                                  Value = col_number(),
                                                  Variabel = col_skip(),
                             ...
                             na = "..", 
                                 trim_ws = TRUE) %>%
  rename(v23_netto_driftsutgifter_244_1000kr = Value)
...
temp = full_join(temp, temp_tabell_16, by = c("Region", "Tid")) %>%
  full_join(., temp_tabell_23, by = c("Region", "Tid"))
...
temp = mutate(temp,
                    Value =
                      ifelse(is.na(Value) == TRUE, 
                             round((v23_netto_driftsutgifter_244_1000kr / v16_netto_driftsutgifter_1000kr_totalt_barnevern)*100, 1),
                             Value
                      )) %>%
  select(
    Region, 
...
temp = mutate(temp, Variabel = "2D_andel_244utgifter_nettodriftsutgifter")
...
write.csv2(temp, "data_til_pivotering/2D_andel_244utgifter_nettodriftsutgifter.csv", row.names = FALSE)
write.csv2(temp, "data_bearbeida/v24_andel_244utgifter_nettodriftsutgifter.csv", row.names = FALSE)
...
rm(historiske_bydelsnr, historiske_fylkesnr, historiske_kommunekoder,
   ...
   metadata_geografi_alle, metadata_iks, tabell_24_bydel, tabell_24_kommune,
   temp, temp_bydel, temp_omkodet, temp_tabell_16, temp_tabell_23, test
)
