...
#1 laste inn data
tabell_17_kommune <- read_delim("datauttrekk_raw/tabell_17_kommune.csv", ";", escape_double = FALSE, 
...
tabell_17_bydel <- read_delim("datauttrekk_raw/tabell_17_bydel.csv", ";", escape_double = FALSE, 
...
temp = select(tabell_17_kommune, Region = KOKkommuneregion0000, Tid, Value = value) %>%
  bind_rows(., select(tabell_17_bydel, Region = KOKbydelsregion0000, Tid, Value = value))
...
#i temp_omkodet ligger nå summerte tall for alle kommuner som har endret kommunenummer
#dvs. at jeg tar ut NA-verdier fra målfila.
...
temp = full_join(temp, temp_tabell_1, by = c("Region", "Tid")) %>%
  full_join(., temp_tabell_16, by = c("Region", "Tid"))
...
temp = mutate(temp,
                    Value =
                      ifelse(is.na(Value) == TRUE, 
                             round((v16_netto_driftsutgifter_1000kr_totalt_barnevern / v1_antallbarn_0_17)*1000, 0),
                             Value
                      )) %>%
  select(
    Region, 
...
temp = mutate(temp, Variabel = "2A_andel_netto_driftsutgifter_1000kr_totalt_barnevern_pr017")
...
write.csv2(temp, "data_til_pivotering/2A_andel_netto_driftsutgifter_1000kr_totalt_barnevern_pr017.csv", row.names = FALSE)
write.csv2(temp, "data_bearbeida/v17_andel_netto_driftsutgifter_1000kr_totalt_barnevern_pr017.csv", row.names = FALSE)
...
rm(historiske_bydelsnr, historiske_fylkesnr, historiske_kommunekoder,
   ...
   metadata_geografi_alle, metadata_iks, tabell_17_bydel, tabell_17_kommune,
   temp, temp_bydel, temp_omkodet, temp_tabell_1, temp_tabell_16, test
)
