...
tabell_19_kommune <- read_delim("datauttrekk_raw/tabell_19_kommune.csv", ";", escape_double = FALSE, 
...
tabell_19_bydel <- read_delim("datauttrekk_raw/tabell_19_bydel.csv", ";", escape_double = FALSE, 
...
temp = select(tabell_19_kommune, Region = KOKkommuneregion0000, Tid, Value = value) %>%
  bind_rows(., select(tabell_19_bydel, Region = KOKbydelsregion0000, Tid, Value = value))
...
temp = mutate(temp, Variabel = "2B_andel_netto_driftsutgifter_totalt_barnevern_prbarn_i_barnevernet0_22")
...
write.csv2(temp, "data_til_pivotering/2B_andel_netto_driftsutgifter_totalt_barnevern_prbarn_i_barnevernet0_22.csv", row.names = FALSE)
write.csv2(temp, "data_bearbeida/v19_andel_netto_driftsutgifter_totalt_barnevern_prbarn_i_barnevernet0_22.csv", row.names = FALSE)
...
rm(historiske_bydelsnr, historiske_fylkesnr, historiske_kommunekoder,
   ...
   metadata_geografi_alle, metadata_iks, tabell_19_bydel, tabell_19_kommune,
   temp, temp_bydel, temp_omkodet, temp_tabell_16, temp_tabell_18, test
)
