...
tabell_59_kommune <- read_delim("datauttrekk_raw/tabell_59_kommune.csv", ";", escape_double = FALSE, 
...
tabell_59_bydel <- read_delim("datauttrekk_raw/tabell_7_bydel.csv", ";", escape_double = FALSE, 
...
temp = select(tabell_59_kommune, Region = KOKkommuneregion0000, Tid, Value = value) %>%
  bind_rows(., select(tabell_59_bydel, Region = KOKbydelsregion0000, Tid, Value = value))
...
temp_tabell_58 <- read_delim("data_bearbeida/v58_antall_barn_bekymringsmelding.csv", ";", escape_double = FALSE, 
                                 col_types = cols(Region = col_character(),
                                                  Tid = col_character(),
                                                  Value = col_number(),
                                                  Variabel = col_skip(),
                             ...
                             na = "..",
                                 trim_ws = TRUE) %>%
  rename(v58_antall_barn_bekymringsmelding = Value)
...
temp = full_join(temp, temp_tabell_1, by = c("Region", "Tid")) %>%
  full_join(., temp_tabell_58, by = c("Region", "Tid"))
...
temp = mutate(temp,
              Value = ifelse(is.na(Value) == TRUE, 
                             round((v58_antall_barn_bekymringsmelding / v1_antallbarn_0_17)*100, 1),
                             Value
                             )
                      ) %>%
  select(
    ...
    Tid,
    Andel = Value,
    Antall = v58_antall_barn_bekymringsmelding
    ) %>%
...
temp = mutate(temp, Variabel = "4F_andel_barn_med_bekymringsmelding")
...
write.csv2(temp, "data_til_pivotering/4F_andel_barn_med_bekymringsmelding.csv", row.names = FALSE)
write.csv2(temp, "data_bearbeida/v59_andel_barn_med_bekymringsmelding.csv", row.names = FALSE)
...
rm(historiske_bydelsnr, historiske_fylkesnr, historiske_kommunekoder,
   ...
   metadata_geografi_alle, metadata_iks, tabell_59_bydel, tabell_59_kommune,
   temp, temp_bydel, temp_omkodet, temp_tabell_1, temp_tabell_58, test
)
