...
#ANDEL - MÅ BEREGNES SELV (LAGRES UTEN ANTALL)
...
#v26: v16_netto_driftsutgifter_1000kr_totalt_barnevern / v25_netto_driftsutgifter_totalt_kommunen_1000kr
...
temp_tabell_25 <- read_delim("data_bearbeida/v25_netto_driftsutgifter_totalt_kommunen_1000kr.csv", ";", escape_double = FALSE, 
                                 col_types = cols(Region = col_character(),
                                                  Tid = col_character(),
                                                  Value = col_number(),
                                                  Variabel = col_skip(),
                             ...
                             na = "..",   
                                 trim_ws = TRUE) %>%
  rename(v25_netto_driftsutgifter_totalt_kommunen_1000kr = Value)
...
temp = full_join(temp_tabell_16, temp_tabell_25, by = c("Region", "Tid")) 
...
temp = mutate(temp,
                    Value = round((v16_netto_driftsutgifter_1000kr_totalt_barnevern / v25_netto_driftsutgifter_totalt_kommunen_1000kr)*100, 1)
                      ) %>%
  select(
    Region, 
    Tid, 
...
temp = mutate(temp, Variabel = "2E_andel_netto_driftsutgifter_barnevern__av_totale_driftsutgifter")
...
write.csv2(temp, "data_til_pivotering/2E_andel_netto_driftsutgifter_barnevern__av_totale_driftsutgifter.csv", row.names = FALSE)
write.csv2(temp, "data_bearbeida/v26_andel_netto_driftsutgifter_barnevern__av_totale_driftsutgifter.csv", row.names = FALSE)

#rydder opp tabeller som ikke trenger å være i minnet lengre
rm(temp, temp_tabell_16, temp_tabell_25, test)
