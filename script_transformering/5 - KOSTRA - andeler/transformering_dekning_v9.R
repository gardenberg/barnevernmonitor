...
#9	Andel barn med hjelpetiltak i hjemmet av alle barn med tiltak 0-17 år
# Til 1C Barn som bor utenfor hjemmet i løpet av året i forhold til antall innbyggere i aldersgruppen 0-17 år
# Beregnes med nr. 8 og nr. 2. - ingen kilde til denne.
...
temp_tabell_8 <- read_delim("data_bearbeida/v8_barn_tiltak_ilaaret_251_0_17.csv", ";", escape_double = FALSE, 
                                 col_types = cols(Region = col_character(),
                                                  Tid = col_character(),
                                                  Value = col_number(),
                                                  Variabel = col_skip(),
                            ...
                            na = "..",
                                 trim_ws = TRUE) %>%
  rename(v8_barn_tiltak_ilaaret_251_0_17 = Value)
...
temp = full_join(temp_tabell_8, temp_tabell_2, by = c("Region", "Tid"))
...
temp = mutate(temp,
                    Value =
                      round((v8_barn_tiltak_ilaaret_251_0_17 / v2_barn_tiltak_ilaaret_ialt_0_17)*100, 1),
                             Value
                      ) %>%
  select(
    Region, 
    Tid, 
    Andel = Value,
    Antall = v8_barn_tiltak_ilaaret_251_0_17) %>%
...
temp = mutate(temp, Variabel = "1E_andel_barn_tiltak_ilaaret_251_0_17")
...
write.csv2(temp, "data_til_pivotering/1E_andel_barn_tiltak_ilaaret_251_0_17.csv", row.names = FALSE)
write.csv2(temp, "data_bearbeida/v9_andel_barn_tiltak_ilaaret_251_0_17.csv", row.names = FALSE)

#rydder opp tabeller som ikke trenger å være i minnet lengre
rm(temp, temp_tabell_2, temp_tabell_8, test)
