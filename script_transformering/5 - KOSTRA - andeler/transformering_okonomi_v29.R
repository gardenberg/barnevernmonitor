...
#ANDEL - KOSTRA (PRESENTERES UTEN ANTALL)
...
tabell_29_kommune <- read_delim("datauttrekk_raw/tabell_29_kommune.csv", ";", escape_double = FALSE, 
...
tabell_29_bydel <- read_delim("datauttrekk_raw/tabell_29_bydel.csv", ";", escape_double = FALSE, 
...
temp = select(tabell_29_kommune, Region = KOKkommuneregion0000, Tid, Value = value) %>%
  bind_rows(., select(tabell_29_bydel, Region = KOKbydelsregion0000, Tid, Value = value))
...
#HVIS RELEVANT
temp_tabell_27 <- read_delim("data_bearbeida/v27_barn_022_251_ilaaar.csv", ";", escape_double = FALSE, 
                             ...
                             na = "..",  
                                trim_ws = TRUE) %>%
  rename(v27_barn_022_251_ilaaar = Value)

temp_tabell_28 <- read_delim("data_bearbeida/v28_brutto_driftsutgifter_251_1000kr.csv", ";", escape_double = FALSE, 
                                 col_types = cols(Region = col_character(),
                                                  Tid = col_character(),
                                                  Value = col_number(),
                                                  Variabel = col_skip(),
                             ...
                             na = "..",   
                                 trim_ws = TRUE) %>%
  rename(v28_brutto_driftsutgifter_251_1000kr = Value)
...
temp = full_join(temp, temp_tabell_27, by = c("Region", "Tid")) %>%
  full_join(., temp_tabell_28, by = c("Region", "Tid"))
...
temp = mutate(temp,
                    Value =
                      ifelse(is.na(Value) == TRUE, 
                             round((v28_brutto_driftsutgifter_251_1000kr / v27_barn_022_251_ilaaar)*1000, 0),
                             Value
                      )) %>%
  select(
    Region, 
...
temp = mutate(temp, Variabel = "2F_brutto_driftsutgifter_251_prbarn_ihjemmet")
...
write.csv2(temp, "data_til_pivotering/2F_brutto_driftsutgifter_251_prbarn_ihjemmet.csv", row.names = FALSE)
write.csv2(temp, "data_bearbeida/v29_brutto_driftsutgifter_251_prbarn_ihjemmet.csv", row.names = FALSE)
...
rm(historiske_bydelsnr, historiske_fylkesnr, historiske_kommunekoder,
   ...
   metadata_geografi_alle, metadata_iks, tabell_29_bydel, tabell_29_kommune,
   temp, temp_bydel, temp_omkodet, temp_tabell_27, temp_tabell_28, test
)
