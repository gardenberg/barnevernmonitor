...
# Til #1C Barn som bor utenfor hjemmet i løpet av året i forhold til antall innbyggere i aldersgruppen 0-17 år
...
tabell_7_kommune <- read_delim("datauttrekk_raw/tabell_7_kommune.csv", ";", escape_double = FALSE, 
...
tabell_7_bydel <- read_delim("datauttrekk_raw/tabell_7_bydel.csv", ";", escape_double = FALSE, 
...
temp = select(tabell_7_kommune, Region = KOKkommuneregion0000, Tid, Value = value) %>%
  bind_rows(., select(tabell_7_bydel, Region = KOKbydelsregion0000, Tid, Value = value))
...
temp_tabell_6 <- read_delim("data_bearbeida/v6_barn_tiltak_ilaaret_252_0_17.csv", ";", escape_double = FALSE, 
                                 col_types = cols(Region = col_character(),
                                                  Tid = col_character(),
                                                  Value = col_number(),
                                                  Variabel = col_skip(),
                            ...
                            na = "..",
                                 trim_ws = TRUE) %>%
  rename(v6_barn_tiltak_ilaaret_252_0_17 = Value)
...
temp = full_join(temp, temp_tabell_1, by = c("Region", "Tid")) %>%
  full_join(., temp_tabell_6, by = c("Region", "Tid"))
...
temp = mutate(temp,
                    Value =
                      ifelse(is.na(Value) == TRUE, 
                             round((v6_barn_tiltak_ilaaret_252_0_17 / v1_antallbarn_0_17)*100, 1),
                             Value
                      )) %>%
  select(
    Region, 
    Tid, 
    Andel = Value,
    Antall = v6_barn_tiltak_ilaaret_252_0_17) %>%
...
temp = mutate(temp, Variabel = "1C_andel_barn_tiltak_ilaaret_252_0_17")
...
write.csv2(temp, "data_til_pivotering/1C_andel_barn_tiltak_ilaaret_252_0_17.csv", row.names = FALSE)
write.csv2(temp, "data_bearbeida/v7_andel_barn_tiltak_ilaaret_252_0_17.csv", row.names = FALSE)
...
rm(historiske_bydelsnr, historiske_fylkesnr, historiske_kommunekoder,
   ...
   metadata_geografi_alle, metadata_iks, tabell_7_bydel, tabell_7_kommune,
   temp, temp_bydel, temp_omkodet, temp_tabell_1, temp_tabell_6, test
)
