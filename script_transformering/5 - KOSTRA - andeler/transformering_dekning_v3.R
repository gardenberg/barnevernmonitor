...
# Barn med barnevernstiltak i forhold til innbyggere i aldersgruppen 0-17 år https://www.ssb.no/statbank/table/12604/ (B), https://www.ssb.no/statbank/table/12275/ (K)
# TIL 1A Barn med barnevernstiltak i forhold til innbyggere i aldersgruppen 0-17 år
...
tabell_3_kommune <- read_delim("datauttrekk_raw/tabell_3_kommune.csv", ";", escape_double = FALSE, 
                               col_types = cols(KOKkommuneregion0000 = col_character(),
                                                KOKalder0000 = col_character(),
                                                KOKbvfunksjon0000 = col_character(),
...
tabell_3_bydel <- read_delim("datauttrekk_raw/tabell_3_bydel.csv", ";", escape_double = FALSE, 
...
temp = select(tabell_3_kommune, Region = KOKkommuneregion0000, Tid, Value = value) %>%
  bind_rows(., select(tabell_3_bydel, Region = KOKbydelsregion0000, Tid, Value = value))
...
temp_tabell_2 <- read_delim("data_bearbeida/v2_barn_tiltak_ilaaret_ialt_0_17.csv", ";", escape_double = FALSE, 
                                 col_types = cols(Region = col_character(),
                                                  Tid = col_character(),
                                                  Value = col_number(),
                                                  Variabel = col_skip(),
                            ...
                            na = "..",
                                 trim_ws = TRUE) %>%
  rename(v2_barn_tiltak_ilaaret_ialt_0_17 = Value)
...
temp = full_join(temp, temp_tabell_1, by = c("Region", "Tid")) %>%
  full_join(., temp_tabell_2, by = c("Region", "Tid"))
...
temp = mutate(temp,
                    Value =
                      ifelse(is.na(Value) == TRUE, 
                             round((v2_barn_tiltak_ilaaret_ialt_0_17 / v1_antallbarn_0_17)*100, 1),
                             Value
                      )) %>%
  select(
    Region, 
    Tid, 
    Andel = Value,
    Antall = v2_barn_tiltak_ilaaret_ialt_0_17) %>%
...
temp = mutate(temp, Variabel = "1A_andel_barn_tiltak_ilaaret_ialt_0_17")
...
write.csv2(temp, "data_til_pivotering/1A_andel_barn_tiltak_ilaaret_ialt_0_17.csv", row.names = FALSE)
write.csv2(temp, "data_bearbeida/v3_andel_barn_tiltak_ilaaret_ialt_0_17.csv", row.names = FALSE)
...
rm(historiske_bydelsnr, historiske_fylkesnr, historiske_kommunekoder,
   ...
   metadata_geografi_alle, metadata_iks, tabell_3_bydel, tabell_3_kommune,
   temp, temp_bydel, temp_omkodet, temp_tabell_1, temp_tabell_2, test
)
