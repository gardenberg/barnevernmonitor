...
tabell_48 <- read_delim("datauttrekk_raw/tabell_48.csv", ";", escape_double = FALSE, 
...
temp = select(tabell_48, -ContentsCode) %>%
  group_by(Region, Tid) %>%
...
#for tabell 1 og 45, 47 returnerer API-et 0 for enheter som ikke lenger finnes 
...
temp = mutate(temp, 
              Variabel = "v48_antall_enslige_forsørgere_totalt",
              Tallformat = "Antall"
)
...
write.csv2(temp, "data_bearbeida/v48_antall_enslige_forsørgere_totalt.csv", row.names = FALSE)
...
rm(historiske_bydelsnr, historiske_fylkesnr, historiske_kommunekoder,
   ...
   metadata_geografi_alle, metadata_iks, tabell_47, tabell_48,
