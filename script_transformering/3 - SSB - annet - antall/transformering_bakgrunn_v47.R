...
tabell_47 <- read_delim("datauttrekk_raw/tabell_47.csv", ";", escape_double = FALSE, 
...
temp = select(tabell_47, -ContentsCode) %>%
  group_by(Region, Tid) %>%
...
temp = mutate(temp, 
              Variabel = "v47_antall_enslige_forsørgere_mbarn",
              Tallformat = "Antall"
)
...
write.csv2(temp, "data_bearbeida/v47_antall_enslige_forsørgere_mbarn.csv", row.names = FALSE)
...
rm(historiske_bydelsnr, historiske_fylkesnr, historiske_kommunekoder,
   ...
   metadata_geografi_alle, metadata_iks, tabell_47,
