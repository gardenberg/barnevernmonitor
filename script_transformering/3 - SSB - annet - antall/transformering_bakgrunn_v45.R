...
tabell_45_kommune <- read_delim("datauttrekk_raw/tabell_45_kommune.csv", ";", escape_double = FALSE, 
                               col_types = cols(Region = col_character(),
                                                Tid = col_character()), 
                               locale = locale(decimal_mark = ",", grouping_mark = " "), 
                               trim_ws = TRUE)

tabell_45_bydel <- read_delim("datauttrekk_raw/tabell_45_bydel.csv", ";", escape_double = FALSE, 
                               col_types = cols(Region = col_character(),
                                                Tid = col_character()), 
                               locale = locale(decimal_mark = ",", grouping_mark = " "), 
                               trim_ws = TRUE)
...
temp = bind_rows(tabell_45_kommune, tabell_45_bydel)

temp = select(temp, Region, Tid, Value = value)
...
#for tabell 1 og 45 returnerer API-et 0 for enheter som ikke lenger finnes 
...
temp = mutate(temp, 
              Variabel = "v45_antall_innbyggere_totalt",
              Tallformat = "Antall"
)
...
write.csv2(temp, "data_bearbeida/v45_antall_innbyggere_totalt.csv", row.names = FALSE)
...
rm(historiske_bydelsnr, historiske_fylkesnr, historiske_kommunekoder,
   ...
   metadata_geografi_alle, metadata_iks, tabell_45_bydel, tabell_45_kommune,
