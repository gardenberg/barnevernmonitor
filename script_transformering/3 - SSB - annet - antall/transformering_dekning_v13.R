...
tabell_13_fylker <- read_delim("datauttrekk_raw/tabell_13_fylker.csv", ";", escape_double = FALSE, 
                               col_types = cols(
...
tabell_13_kommuner_2013_2018 <- read_delim("data_manuelle_leveranser/tabell_13_14_kommuner_2013_2018.csv", ";", escape_double = FALSE, 
                                           locale = locale(decimal_mark = ",", grouping_mark = "|", encoding = "ISO-8859-1"),
                                           col_types = cols(
                                             Tid = col_character(),
                                             Region = col_character(),
                                             fosterheim_familie_nare_nettverk = col_number(),
                                             fosterheim_utenom_familie_nare_nettverk = col_skip()
                                           ), 
                                           trim_ws = TRUE)

tabell_13_kommuner_2019 <- read_delim("data_manuelle_leveranser/tabell_13_14_eksempel dataleveranse fra SSB - fosterhjem i slekt og nettverk 2019.csv", ";", escape_double = FALSE, 
                                           locale = locale(decimal_mark = ",", grouping_mark = "|", encoding = "ISO-8859-1"),
                                           col_types = cols(
                                             kommunenummer = col_character(),
                                             kommune = col_skip(),
                                             fosterheim_familie_nettverk_ilaaaret_2019 = col_number(), #verdt å sjekke i kilden hvordan NA, : . er håndtert fra SSB
                                             fosterheim_utenom_familie_nettverk_ilaaaret_2019 = col_skip(),
...
temp = select(tabell_13_fylker, Region, Tid, Value = value) %>%
  bind_rows(., select(tabell_13_kommuner_2013_2018, Region, Tid, Value = fosterheim_familie_nare_nettverk)) %>%
  bind_rows(., select(tabell_13_kommuner_2019, Region = kommunenummer, Tid, Value = fosterheim_familie_nettverk_ilaaaret_2019))
...
temp = mutate(temp, 
              Variabel = "v13_fosterheim_familie_nare_nettverk",
              Tallformat = "Antall"
)
...
write.csv2(temp, "data_bearbeida/v13_fosterheim_familie_nare_nettverk.csv", row.names = FALSE)
...
rm(fjerneliste, historiske_bydelsnr, historiske_fylkesnr, historiske_kommunekoder,
   ...
   metadata_geografi_alle, metadata_iks, tabell_13_fylker, tabell_13_kommuner_2013_2018, tabell_13_kommuner_2019,
