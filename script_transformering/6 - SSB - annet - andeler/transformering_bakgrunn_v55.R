...
#Dette er steg 1 av den andelsberegningen
...
#versjon 002
#først vurdert som edgecase, men så ikke allikevel
...
tabell_55 <- read_delim("datauttrekk_raw/tabell_55.csv", ";", escape_double = FALSE, 
...
temp = select(tabell_55, Region, Tid, Value = value)
...
#for lavinntektstabellen legger vi inn en ekstra sjekk av manglende observasjoner
#ettersom det er ganske mange ting som mangler her i utgangspunktet
manglende_observasjoner = select(metadata_episerver, Region = code) %>%
  left_join(., temp) %>%
  expand(., Region, Tid) %>%
  filter(is.na(Tid) == FALSE) %>%
  anti_join(., temp)
...
temp = mutate(temp, 
              Variabel = "4E_andel_barn_lavinntekt_steg1",
              Tallformat = "Andel"
              )

#skriver ut til ferdig bearbeida data
write.csv2(temp, "data_bearbeida/v55_andel_barn_lavinntekt.csv", row.names = FALSE)
write.csv2(temp, "data_til_pivotering/4E_andel_barn_lavinntekt.csv", row.names = FALSE)

#rydder opp tabeller som ikke trenger å være i minnet lengre
rm(historiske_bydelsnr, historiske_fylkesnr, historiske_kommunekoder, historiske_kommunekoder_gyldig, 
   manglende_observasjoner, metadata_episerver, metadata_geografi_alle, metadata_iks,
   tabell_55, temp, temp_bydel, temp_omkodet, test, checksum_start, checksum_end)