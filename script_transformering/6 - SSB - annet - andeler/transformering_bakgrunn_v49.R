...
#HVIS RELEVANT
temp_tabell_47 <- read_delim("data_bearbeida/v47_antall_enslige_forsørgere_mbarn.csv", ";", escape_double = FALSE, 
                             ...
                             na = "..", 
                                trim_ws = TRUE) %>%
  rename(v47_antall_enslige_forsørgere_mbarn = Value)

temp_tabell_48 <- read_delim("data_bearbeida/v48_antall_enslige_forsørgere_totalt.csv", ";", escape_double = FALSE, 
                                 col_types = cols(Region = col_character(),
                                                  Tid = col_character(),
                                                  Value = col_number(),
                                                  Variabel = col_skip(),
                             ...
                             na = "..", 
                                 trim_ws = TRUE) %>%
  rename(v48_antall_enslige_forsørgere_totalt = Value)
...
temp = full_join(temp_tabell_47, temp_tabell_48, by = c("Region", "Tid"))
...
temp = mutate(temp,
                    Value =
                      round((v47_antall_enslige_forsørgere_mbarn / v48_antall_enslige_forsørgere_totalt)*100, 1)
                      ) %>%
  select(
    Region, 
    Tid,
    Andel = Value,
    Antall = v47_antall_enslige_forsørgere_mbarn) %>%
...
temp = mutate(temp, Variabel = "4B_andel_enslige_forsørgere_mbarn")
...
write.csv2(temp, "data_til_pivotering/4B_andel_enslige_forsørgere_mbarn.csv", row.names = FALSE)
write.csv2(temp, "data_bearbeida/v49_andel_enslige_forsørgere_mbarn.csv", row.names = FALSE)

#rydder opp tabeller som ikke trenger å være i minnet lengre
rm(temp, temp_tabell_47, temp_tabell_48, test)
